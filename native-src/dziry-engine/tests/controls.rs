...
/// Keyboard operability is the half of accessibility dziry claims — ROADMAP's Accessibility
...
mod keyboard {
    ...
    ///
    /// **This is the honest limit of dziry's keyboard story and it deserves a comment rather
    ...
    fn committing_reports_the_select_and_the_chosen_index() {
        ...
        let mut out = [dziry_engine::engine::Event::default(); 16];
        ...
        // And the CLICK still names the row, because dziry has no bubbling — the two
...
mod tab_order {
    ...
    /// one stop on `BODY` before the cycle restarted — and dziry has no chrome to hand it
...
mod group_arrows {
    ...
    /// Up. A browser does not ask how the group was laid out, and neither can dziry:
...
mod autofocus {
    ...
    /// The latch, and the reason it exists is dziry's rather than the browser's.
    ///

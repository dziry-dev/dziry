...
use dziry_engine::engine::EngineConfig;
use dziry_engine::protocol::{self, status};
use dziry_engine::tables::SpanDesc;
use dziry_engine::*;
...
fn last_error() -> String {
    unsafe {
        let len = dziry_last_error(std::ptr::null_mut(), 0);
        let mut buf = vec![0u8; len as usize];
        dziry_last_error(buf.as_mut_ptr(), len);
...
fn create() -> Handle {
    ...
    let code = unsafe { dziry_engine_create(&config(), &mut handle) };
...
fn the_host_can_ask_what_protocol_this_binary_speaks() {
    assert_eq!(dziry_protocol_version(), protocol::PROTOCOL_VERSION);
}
...
fn a_panic_becomes_a_status_code_and_poisons_the_engine() {
    ...
    let code = dziry_engine_panic_for_testing(handle);
    ...
    assert_eq!(dziry_engine_tick(handle), status::POISONED);

    dziry_engine_destroy(handle);
}

#[test]
fn a_destroyed_handle_is_refused_rather_than_dereferenced() {
    let handle = create();
    assert_eq!(dziry_engine_destroy(handle), status::OK);
    ...
    assert_eq!(dziry_engine_destroy(handle), status::INVALID_HANDLE);
    assert!(
        last_error().contains("already destroyed"),
        "the message should say which mistake this was: {}",
        last_error()
    );
    assert_eq!(dziry_engine_tick(handle), status::INVALID_HANDLE);
}
...
fn a_wrong_generation_on_a_live_slot_is_refused() {
    ...
    assert_eq!(dziry_engine_tick(live), status::OK, "{}", last_error());
    assert_eq!(dziry_engine_destroy(live), status::OK);
}

#[test]
fn a_zero_handle_is_an_error_not_a_crash() {
    assert_eq!(dziry_engine_tick(0), status::INVALID_HANDLE);
    assert_eq!(
        dziry_engine_destroy(0),
        status::OK,
    ...
    assert_eq!(dziry_engine_tick(0xdead_beef), status::INVALID_HANDLE);
}
...
fn a_handle_used_from_another_thread_is_refused() {
    ...
    let code = std::thread::spawn(move || dziry_engine_tick(handle))
        .join()
    ...
    assert_eq!(dziry_engine_tick(handle), status::OK, "{}", last_error());
    assert_eq!(dziry_engine_destroy(handle), status::OK);
}
...
fn a_short_descriptor_buffer_is_refused_before_it_is_written() {
    ...
    let mut count = 0u32;
    assert_eq!(
        unsafe { dziry_engine_span_count(handle, &mut count) },
        status::OK
    );
    assert!(count > 0);
    ...
    let code = unsafe { dziry_engine_describe(handle, one.as_mut_ptr(), 1, &mut written) };
...
fn the_descriptor_survives_a_round_trip_through_the_abi() {
    ...
    let code = unsafe { dziry_engine_describe(handle, spans.as_mut_ptr(), count, &mut written) };
    ...
    unsafe { dziry_engine_generation(handle, &mut generation) };
    assert!(generation > 0);
...
fn a_skia_failure_reports_skia_and_not_the_entry_point_s_guess() {
    // `dziry_engine_resize` used to map every failure beneath it to `SDL`,
    ...
    let handle = create();
    let code = dziry_engine_resize(handle, u32::MAX, u32::MAX);
...
fn a_short_png_buffer_leaves_the_frame_to_retry() {
    ...
    let mut size = 0u32;
    assert_eq!(
        unsafe { dziry_engine_encode_png(handle, &mut size) },
        status::OK,
        "{}",
        last_error()
    );
    ...
    assert_eq!(
        unsafe { dziry_engine_take_png(handle, short.as_mut_ptr(), short.len() as u32) },
        status::CAPACITY,
    );
    assert!(
    ...
    assert_eq!(
        unsafe { dziry_engine_take_png(handle, png.as_mut_ptr(), png.len() as u32) },
        status::OK,
        "{}",
        last_error()
    );
...
fn a_short_font_family_buffer_reports_what_it_wrote() {
    ...
    let mut needed = 0u32;
    assert_eq!(
        unsafe { dziry_engine_font_family(handle, std::ptr::null_mut(), 0, &mut needed) },
        status::OK
    );
    ...
    let mut written = 99u32;
    assert_eq!(
        unsafe { dziry_engine_font_family(handle, buf.as_mut_ptr(), 4, &mut written) },
        status::OK
    );
...
fn a_headless_engine_paints_pixels() {
    ...
    let mut info = [0u32; 4];
    assert_eq!(
        unsafe { dziry_engine_surface_info(handle, info.as_mut_ptr()) },
        status::OK
    );
    assert_eq!(info[0], 64);
    assert_eq!(info[1], 64);
    ...
    assert_eq!(
        unsafe { dziry_engine_read_pixels(handle, pixels.as_mut_ptr(), pixels.len() as u32) },
        status::OK
    );

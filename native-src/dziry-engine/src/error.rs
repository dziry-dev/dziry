...
thread_local! {
    ...
    /// `dziry_last_error`, cleared by the next successful call that sets it.
...
/// through `dziry_last_error`, and an error hierarchy would buy nothing across
...
///
/// Shared by `dziry_last_error` and `dziry_engine_font_family`, which are the only

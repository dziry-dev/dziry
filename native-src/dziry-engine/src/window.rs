...
/// it is the width a browser team settled on for "a page still works here", and dziry's
...
///
/// `DZIRY_MIN_WINDOW=none` removes it; `DZIRY_MIN_WINDOW=200x150` moves it.
///
...
const MIN_WINDOW_ENV: &str = "DZIRY_MIN_WINDOW";
...
impl Window {
    pub fn new(title: &str, width: u32, height: u32, decorated: bool) -> Result<Self, EngineError> {
        ...
        // — *unless* `DZIRY_MIN_WINDOW` removed it, which is why that is a measurement
    ...
    /// dziry* would be the one part of the app that does not look like the system.
    ///
...
/// drains it on the next frame via `dziry_engine_take_file_dialog_result`.

...
/// rasterisation deal with it. Rounding cost dziry a visible bug rather than just
...
impl LayoutTree {
    ...
    /// `ua-sheet.ts`, matched through the `data-dziry-listbox` attribute the compiler
...
fn style_of(tables: &Tables, node: usize, globals: u32, surface: [f32; 2]) -> Style {
    ...
    //
    // dziry has no `box-sizing` *field* yet, so this sets the initial value rather

...
pub struct Event {
    ...
    /// prevent — so `dziry_engine_event_size` reports `size_of::<Event>()` and `host.ts`
...
pub struct Engine {
    ...
    /// `autofocus` inserted after load moves nothing — but dziry needs it for a reason a
...
impl Engine {
    ...
    ///
    /// Keyboard operability is the half of accessibility dziry claims (see ROADMAP's
    ...
    ///
    /// The one thing a browser has here that dziry does not: its synthesised click carries
    /// `detail: 0` where a pointer's carries 1, which is how libraries tell them apart.
    /// dziry's `CLICK` has no such field, so the two are **indistinguishable to a host**.
    ...
    ///
    /// The events it queues are the measured pair. `CLICK` on the **option**, because dziry
    ...
    /// beside the event, from `dziry_engine_listbox_selection`, which is the same answer
    /// `dziry_engine_selection` gives for a text range: engine state with no signal to hold
    /// it.
    ...
    ///
    /// The accessor behind `dziry_engine_listbox_selection`. Indices rather than node ids
    ...
    /// navigating fires neither. dziry emits `CHANGE` and not yet an `INPUT` — `onInput` is
    ...
    fn choose_option(&mut self, option: i32, x: f32, y: f32) {
        ...
        if act.changed {
            ...
            // stays on the option deliberately — dziry has no bubbling, so the node a click

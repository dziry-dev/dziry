...
///
/// dziry's scrollbars are **overlay**: drawn over the content, reserving no layout
...
/// `scrollbar-width: thin`, against dziry's own default rather than a native gutter.
///
...
/// default. Copying Chromium's 10 would be wider than dziry's `auto`, which is the one
...
impl BarPhase {
    ...
    fn thickness(self, width: u8) -> f32 {
        let base = match width {
            ...
            // dziry's own overlay thickness rather than a native gutter width.
            _ => THUMB_THICKNESS,
        };
        match self {
            BarPhase::Idle => base,
...
fn resolve_slot(
    ...
    //
    // Itself, except for two kinds of node that have no state of their own:
    //
    // - a box generated by `::before` / `::after` reads its parent's.
    //   `.btn:hover::before` means "the generated box of a hovered button", and
    //   the box is never `state.hovered` itself — `hit_test` only ever returns
    //   `INTERACTIVE` nodes, and a generated box is not one. The parent *is* the
    //   originating element by construction. See `NodeFlags.GENERATED`.
    //
    // - a TEXT run reads its element's. A run's variant rows are the *projection*
    //   of its element's — the compiler emits them only where a predicate changes
    //   an inherited text field — so `.done:checked`, `.field:invalid { color }`
    //   and `a:hover { color }` recolour the text the element actually shows.
    //   Two hops for a pseudo-element's own text (`content: "✓"`): the run's
    //   parent is the generated box, whose subject is the element.
    //
    // Without either redirect the compiled variant exists and nothing can ever
    // select it — a rule that silently does nothing rather than a visible fault.
    let flags_col = tables.u8s(NODES, protocol::nodes::FLAGS);
    let kinds = tables.u8s(NODES, protocol::nodes::KIND);
    let parents = tables.i32s(NODES, protocol::nodes::PARENT);

    let mut subject = i;
    if kinds.get(node).copied().unwrap_or(0) == protocol::node_kind::TEXT {
        subject = parents.get(node).copied().unwrap_or(i);
    }
    if subject >= 0
        && flags_col.get(subject as usize).copied().unwrap_or(0) & protocol::flags::GENERATED != 0
    {
        subject = parents.get(subject as usize).copied().unwrap_or(subject);
    }
    let subject = subject;
    ...
    // focus, and dziry has neither it nor any reason to change this line.
    //
...
impl Painter {
    ...
    fn scrollbars(
        ...
        for bar in [bar_y, bar_x].into_iter().flatten() {
            ...
            // itself dziry draws a thumb and nothing else — a track would have to be a
...
mod tests {
    ...
    /// A text run reads its *element's* state — here the projection of `Predicate.ROW`.
    ///
    /// `.done { color }` compiles the run's two rows at build time; the bit lives on
    /// the element's control row, written by the host's list update, and the run is a
    /// TEXT node with no state of its own. Without the redirect the compiled variant
    /// existed and nothing could ever select it — the same silent shape the generated
    /// box had before its own redirect.
    #[test]
    fn a_text_run_wears_its_elements_row_style() {
        let mut tables = Tables::new(Capacities {
            nodes: 4,
            styles: 4,
            variants: 1,
        ...
        let variants = Table::Variants as usize;
        let slots = Table::VariantSlots as usize;
        let controls = Table::Controls as usize;

        // Node 0 is the element; node 1 is its text run.
        tables.set_u16(nodes, protocol::nodes::STYLE, 0, 0);
        tables.set_u16(nodes, protocol::nodes::STYLE, 1, 1);
        tables.set_i32(nodes, protocol::nodes::PARENT, 0, -1);
        tables.set_i32(nodes, protocol::nodes::PARENT, 1, 0);
        tables.set_u8(nodes, protocol::nodes::KIND, 1, protocol::node_kind::TEXT);

        tables.set_i32(variants, protocol::variants::NODE, 0, 1);
        tables.set_u32(variants, protocol::variants::MASK, 0, predicate::ROW);
        tables.set_i32(variants, protocol::variants::RUN_START, 0, 0);
        tables.set_u16(slots, protocol::variant_slots::STYLE, 0, 1); // class off
        tables.set_u16(slots, protocol::variant_slots::STYLE, 1, 3); // class on

        // The element's control row, the data-written bit still off.
        tables.set_i32(controls, protocol::controls::NODE, 0, 0);
        tables.set_u8(controls, protocol::controls::FLAGS, 0, 0);
        tables.commit();

        let mut painter = Painter::new();
        let idle = InputState::none();
        painter.rescan_animations(&tables, 2);
        painter.set_input_chains(&tables, &idle);
        assert_eq!(painter.style_for(&tables, 1, &idle), 1);

        // The list update writes the bit; rescan re-reads it; the run follows.
        tables.set_u8(controls, protocol::controls::FLAGS, 0, control_flags::ROW);
        tables.commit();
        painter.rescan_animations(&tables, 2);
        painter.set_input_chains(&tables, &idle);
        assert_eq!(
            painter.style_for(&tables, 1, &idle),
            3,
            "the run must wear the class-on row while its element's data bit is set"
        );
    }
    ...
    fn the_composed_matrix_matches_chromium() {
        ...
        // pins which of the two dziry applies first.
        let mut t = one_style();
        at_origin(&mut t);

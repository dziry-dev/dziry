...
test("two disjoint toggles compose for every combination", async () => {
  ...
  const mismatches = await verifyCompose(doc, css, compileTree(doc, css), togglesOf(doc));
  expect(mismatches).toEqual([]);
});

test("two toggles writing the same field compose when the cascade agrees", async () => {
  ...
  const compiled = await compileVariants(doc, css, compileTree(doc, css), toggles);
  expect(compiled.warnings.join(" ")).toContain("style field(s) in common");

  expect(await verifyCompose(doc, css, compileTree(doc, css), toggles)).toEqual([]);
});

test("a toggle that introduces a pseudo-state composes under that state too", async () => {
...
test("a deliberately broken patch is caught", async () => {
  ...
  const mismatches = await verifyCompose(doc, css, compileTree(doc, css), toggles);
...
test("compiling with no toggles has nothing to verify", async () => {
  const html = `<body><div class="x"></div></body>`;
  const css = `.x { width: 5px }`;
  expect(await verifyCompose(parseHtml(html), css, compile(html, css), [])).toEqual([]);
});

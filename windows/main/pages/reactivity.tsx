 ...
 */
// `signal` is imported even though the rewrite replaces the call with
// `__dzr.local(…)` before it ever runs: the author writes `signal(0)`, so the
// name has to resolve, and `tsc` is reading the source rather than the rewrite.
import { cn, signal } from 'dziri';
import {
    addLang,
    bump,
    doubled,
        ...
        <div className="flex flex-col gap-5">
                ...
                <div className="flex flex-row items-center justify-between">
                    <div className="flex flex-col gap-1">
                        ...
                        <div className="muted text-xs text-zinc-400">
                            five things · signal · computed · .set · .map · cn — and a read is just
                            the name
                        </div>
                    </div>
          ...
          always truthy; a template literal printed [object Object]. */}
            <div className={CARD}>
                ...
                <div className="muted text-xs text-zinc-400">
                    Written in a computed, in the window&apos;s own module. The read is the name —
                    no .value, no dependency array.
                </div>
            ...
            <div className={CARD}>
                ...
                <div className="sunken flex flex-col gap-2 rounded-lg bg-zinc-950 p-4">
                    <div className="text-xs text-zinc-400">
                        ...
                        generated module as text — so it can only name exports. A local cannot be
                        written down: {'{'}`at ${'{'}router.path{'}'}`{'}'} is a build error naming
                        the export it should have used, while {'{'}router.path{'}'} on its own
                        compiles by identity.
                    </div>
                </div>
            </div>
          ...
          just style-table writes when the cell flips. */}
            <div className={CARD}>
                ...
                <div className="muted text-xs text-zinc-400">
                    cn(&quot;box&quot;, {'{'} big: isBig {'}'}) — the class is resolved both ways at
                    build time, so a change costs a few integer writes and nothing per frame.
                </div>

                <div className="flex flex-row items-center gap-3">
                    <div
                        className={cn(
                            'flag rounded-lg bg-zinc-800 px-4 py-2 text-sm text-zinc-100',
                            { active: isBig }
                        )}
                    >
                        tick &gt; 5
                    ...
                    <div className="text-xs text-zinc-500">
                        both driven by computeds over one signal
                    </div>
                </div>
            </div>
            ...
            <div className={CARD}>
                ...
                <div className="flex flex-row gap-2">
                    ...
                    />
                    <button
                        className={cn(BTN, 'bg-sky-600 text-sky-50 hover:bg-sky-500')}
                        onClick={addLang}
                    >
                        Add
                    </button>
                ...
                <div className="flex flex-col gap-2">
                    {langRows.map(
                        (l: Lang & { badge: string }) => (
                            <LangRow badge={l.badge} name={l.name} kind={l.kind} />
                        ),
                        {
                            key: (l: Lang) => l.id
                        }
          ...
          for free and the only missing piece was a name. */}
            ...
            <div className={CARD}>
                ...
                <div className="flex flex-col gap-2">
                    <Row
                        code="tick.set(tick + 1)"
                        value="a value"
                        note="reads work in handlers too"
                    />
                    <Row
                        code="tick.set((n) => n - 1)"
                        value="or a function"
                        note="one method, not two"
                    />
                </div>
            </div>

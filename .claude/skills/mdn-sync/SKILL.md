...
```bash
rg "min-width: auto" guards/vendor/mdn/files/en-us/web/css
rg -l "currentcolor" guards/vendor/mdn/files/en-us/web/css/reference/properties/
```

Pages live at `guards/vendor/mdn/files/en-us/web/css/reference/properties/<name>/index.md`, with YAML
...
seconds, and pinnable to a commit. `guards/vendor/mdn.json` records the SHA so a finding is reproducible.
...
MDN prose is CC-BY-SA. `guards/vendor/mdn/` is gitignored deliberately: vendoring it would attach a

...
callback source that needs nothing. Return an Effect `Stream` and dziri runs it
...
`SDL_ShowSimpleMessageBox` behind an FFI call, so it is not something dziri draws; a dialog
...
`peek` <Status of="peek" /> and `resource` <Status of="resource" /> are designed
but not implemented. They are described in `API.md`; nothing on this page depends
on them.

...
dialect. dziri nests by structure because a compiler can see structure: nothing is parsed at
...
See the [Forms guide](../guide/forms.md) for the whole feature end to end.
...
`:is(:where(.group\/error) *)`, and the `*` inside `:is()` is not a selector dziri parses.
...
dziri depends on none of them: the standard ones are used through their `~standard` property, and
...
Fields inside a `map()` list are not collected — a list template is compiled into an arena rather
than walked, so a `name` inside one is not in the payload. There is also no `form="id"` association,
no file inputs, and a named submit button does not add its own entry.
:::

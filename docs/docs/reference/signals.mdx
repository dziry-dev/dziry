...
The runtime's state primitives. Implemented inside Dziry rather than imported
from a signals library, because the compiler must recognize signals by
...
All of these work, including `===`. A build-time rewrite turns `count === 3`
into `$(count) === 3` — see [Reactivity](../learn/concepts/reactivity.md).
...
[component-local state](../learn/concepts/reactivity.md#component-local-state).
:::
...
[Using Effect](../learn/guides/effect.md#feeding-a-signal-from-a-stream).
...
**The dialog shows the frame that caused it.** The call is queued until the app
thread's next commit, and the engine paints once more before it blocks — so an
alert raised from a handler that also changed the page shows the changed page
behind it, even when both happen inside one `batch()`.

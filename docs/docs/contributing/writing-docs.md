...
reverse direction fails hard: a page teaching `import { foo } from "dziri"` when `foo`
...
resolver `doc-lint` uses. Set `DZIRI_SOURCE_URL` and they become links.
...
- **Document what is done.** Check `API.md` first. A planned surface gets a badge and,
  ...
  that does not exist.
- **Explain the constraint, not just the rule.** Nearly every restriction in dziri

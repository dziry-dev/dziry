...
windows/
  main/
    ...
    index.css        # generated by the Tailwind CLI; an input to our build
...
`dev` runs the real Tailwind CLI over each window's `in.css`, compiles every window
into its `ui.gen.ts` artifact, and opens the app. Anything the CLI does not recognise

...
- **Protocol & host** — The boundary. One schema generates both sides' field identities; the engine reports byte offsets at run time. Everything else is a direct write into shared memory.
  <br>`src/protocol/`, `src/engine/`, `src/host/`, `src/window-host.ts`, `native-src/dziry-engine/src/protocol.rs`, `native-src/dziry-engine/src/tables.rs`, `native-src/dziry-engine/src/lib.rs`
- **Engine** — Rust cdylib: Taffy lays out, Skia paints, SDL3 owns the window and input. It reads Bun-written memory as untrusted input and never lets a panic cross back.
  <br>`native-src/dziry-engine/src/`, `native-src/dziry-engine/examples/`, `native-src/dziry-engine/tests/`, `native-src/dziry-engine/build.rs`
...
`src/protocol/schema.ts`, `src/protocol/generated.ts`, `src/engine/host.ts`, `native-src/dziry-engine/src/protocol.rs`, `native-src/dziry-engine/src/lib.rs`
...
`native-src/dziry-engine/src/lib.rs`, `native-src/dziry-engine/src/error.rs`
...
`native-src/dziry-engine/src/tables.rs`, `native-src/dziry-engine/src/engine.rs`
...
`native-src/dziry-engine/src/layout.rs`
...
`native-src/dziry-engine/src/paint.rs`, `native-src/dziry-engine/src/text.rs`
...
- `bun run layout-diff` — Lays out the same html+css in dziry and in the browser at the same viewport, then compares every box. *(oracle: headless Chrome)*
- `bun run spec-audit` — Checks computed-style defaults and inheritance flags for every style field. *(oracle: mdn-data)*
- `bun run css-coverage` — What CSS exists versus what dziry supports, bucketed as supported / unsupported / committed non-goal. *(oracle: mdn-data)*
- `bun run tailwind-coverage` — What fraction of Tailwind works, and what is blocking the rest ranked by classes unblocked.
- `bun run html-coverage` — How each HTML element renders in dziry versus the browser — the table that specifies the UA stylesheet. *(oracle: headless Chrome)*

...
| per-row conditional classes — `cn({ done: t.done })` | **done** (2026-08-21, protocol v45) — a recorder-valued `cn()` entry compiles like a pseudo-state: both cascades are interned behind `Predicate.ROW`, the element gets a control row to carry the bit, and `updateLists` writes `ControlFlags.ROW` per replica from the row's data. One data-driven class per element (one bit); it styles the element's **own** box — text runs do not follow per-node predicates yet (the same gap `.field:invalid { color }` has), so dim with `opacity`, which composites over the subtree. `row-state.test.tsx` | — |

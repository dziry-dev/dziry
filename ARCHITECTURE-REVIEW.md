...
> loop) and `src/host/window-state.ts` (the state both callers share). Each such finding
> needs re-verifying at the new location before it is acted on.
>
> **`src/compiler/variants.ts` and `src/variants.ts` no longer exist.** This review's own §3
> asked to "point `verifyCompose` at the production `compileVariants` and delete the duplicated
> algorithm". The first half shipped; the second is now done. `compiler/variants.ts` was a second
> implementation of variant compilation reachable only from a CLI, with no test, and — as its own
> comments said — built on a six-role model the shipping compiler had abandoned for a predicate
> bitmask. Findings below citing either file have had their line numbers removed. The shipping
> path is `src/compiler/variant-compile.ts`, whose citations are still live.
>
...
**Where:** `src/protocol/schema.ts:66`, `src/protocol/schema.ts:71`, `src/compiler/compile.ts:727`, `src/compiler/compile.ts:900`, `src/compiler/variants.ts`
...
**Impact.** Any write through `engine.tables` or a retained `Uploader` after `close()` is a write into freed heap — silent corruption of whatever the allocator hands out next, in-process, with no bounds error. Nothing currently triggers it (src/window-host.ts and src/window-host.ts both close at the end of a path), but `#handle = 0` guards only the FFI calls, so the memory path is unguarded while the call path is guarded — the asymmetry is the bug.
...
**Verifier — weakened.** close() does leave every typed-array view live over freed Rust memory and the comment at host.ts:502 is wrong on both counts, but there is no reachable trigger (src/window-host.ts exits immediately, src/window-host.ts is the last statement) and no untrusted-input path, so this is a latent robustness defect rather than a security issue. Severity low.
...
**Where:** `src/runtime/signal.ts:146`, `src/runtime/signal.ts:162`, `src/window-host.ts`
...
**Where:** `src/engine/upload.ts:269`, `src/engine/upload.ts:101`, `src/runtime/list-runtime.ts:146`, `src/window-host.ts`
...
**Evidence.** src/window-host.ts `while (running) { if (dirty) { upload(); dirty = false; } engine.tick(); … for (const event of engine.drainEvents()) {…} await Bun.sleep(8); }` — one `drainEvents()` (default `max = 32`, host.ts:329) per iteration; the "call again while written equals capacity" loop is *not* written. tables.rs:503-519 iterates every span and `if self.staged.as_slice()[range.clone()] == self.live.as_slice()[range.clone()] { continue; }` regardless of whether Bun wrote anything. src/window-host.ts concedes only half: "an idle frame costs an event-queue drain." It also costs a whole-arena memcmp, two FFI round-trips (tick + generation, host.ts:318-327) and a fresh `ArrayBuffer(32*56)` plus `BigUint64Array(1)` per frame.
...
**Impact.** Latent rather than live: the host's discipline masks it (host.ts:394-422 grow → generation bump → src/window-host.ts `uploadAll()` → `uploadStrings(true)` → `#repack()` rewrites every slot). But the invariant the three-arena split rests on — live is what the engine is rendering — is violated for one region across a grow, the next `commit` reports a spurious whole-table `diff.text` and re-measures the entire tree, and any tick between grow and a full re-upload paints empty text.
...
**Impact.** `.btn:hover { padding: 12px }` or `:focus { font-size: 15px }` compiles cleanly, interns a style slot, and produces a label that jumps inside an unchanged box. The compiler already computes `affectsLayout` for pseudo-state deltas (src/compiler/variants.ts) and prints it (src/variants.ts), so the information to reject or honour this exists on both sides and is used on neither.
...
**Evidence.** `state.pressed = hit; state.focused = hit;` on mouse down (engine.rs:434-435) and nothing clears them afterwards except another mouse event. `grow` (engine.rs:514-520) sets `self.fresh = true` and returns — `self.state` is untouched, even though the host's response to a regrow is `uploader.uploadAll()` (src/window-host.ts) which rewrites every node row. `RawInput::FocusChanged` only queues an event (engine.rs:497-501): the window can lose focus mid-press and `pressed` stays set. `WindowEvent::Leave`/mouse-leave is dropped by window.rs:154 `_ => {}`, so `hovered` also survives the cursor leaving the window. KEY_DOWN and TEXT_INPUT then ship the stale id straight to the host (engine.rs:474 `node: self.state.focused`, engine.rs:482 same), where src/window-host.ts resolves it by identity: `editables.find((e) => e.node === focused)`.
...
**Where:** `src/compile.ts:63`, `src/compile.ts:96`, `app/ui.gen.ts:1`, `src/window-host.ts`, `package.json:15`
...
**Where:** `src/compiler/variants.ts`, `src/compiler/variants.ts`, `src/compiler/variant-compile.ts:71`, `src/compiler/variant-compile.ts:252`, `src/variants.ts`
...
**Evidence.** src/compiler/variants.ts: "The real test: does sequencing patches reproduce the compiler's own output for every combination?" … `composeFailures.push({ mask, toggles, mismatches })`. The only consumer is src/variants.ts, which `console.log`s `"    ${p.composeFailures.length}/${a.combos.length} combinations WRONG:"` and exits 0 regardless. And `bun run variants` cannot run: `const inputPath = argv[0] ?? join(ROOT, "app", "todo.tsx")` (src/variants.ts) — `app/` contains only `app.tsx`; I ran it and got `ENOENT: … app\todo.css`. Meanwhile the production path is `compileVariants` (variant-compile.ts:171-303), a near-verbatim reimplementation: `cloneTree` is byte-identical in both files (variants.ts vs variant-compile.ts:71-82), `changedFields` is duplicated (variants.ts vs variant-compile.ts:138), and the slot-vector interning appears a third and fourth time (variants.ts `internSlot`, variant-compile.ts:200 `internSlot`, alongside compile.ts:202 `StyleInterner.intern` and variants.ts `GlobalStyles.intern` — four copies of "build a key by concatenating STYLE_FIELDS"). For contrast, `src/compile.ts` vs `src/compiler/compile.ts` and `src/variants.ts` vs `src/compiler/variants.ts` are *not* duplication — they are CLI-over-library, confirmed by package.json:13-16; the real duplication is variants.ts vs variant-compile.ts.
...
**Recommendation.** Delete the duplicated algorithm. Move the composition check out of `analyzePatches` into a `verifyCompose(doc, css, baseline, toggles): Mismatch[]` that replays `compileVariants`'s own `table` + `patches` against `compileTree` for all 2^k combinations (k is already capped at 16 at variants.ts), and call it from a `src/compiler/variant-compile.test.ts` over `app/app.tsx` plus two or three synthetic fixtures with deliberately colliding toggles. Keep `src/compiler/variants.ts` as a pure *measurement* report (it is honest research and worth keeping) but have it import the production interner instead of restating it, and factor the STYLE_FIELDS key-builder into one exported `styleKey(style)` in `src/ir.ts`. Fix or drop the `app/todo.tsx` default in src/variants.ts.

**Verifier — confirmed.** Two details off: the STYLE_FIELDS key-builder appears three times, not four — compile.ts:206, variants.ts and variant-compile.ts:203; variants.ts `internSlot` keys on combination *style ids*, not on STYLE_FIELDS. And cloneTree is code-identical but not byte-identical: the doc comments differ (variants.ts vs variant-compile.ts:75-76).
...
**Where:** `src/window-host.ts`, `src/engine/upload.test.ts:33`, `src/compiler/compile.ts:881`, `app/ui.gen.ts:1`
...
**Verifier — confirmed.** Cast counts are overstated. There are 17 `as unknown as` in the tree but only 7 on this seam (src/window-host.ts and src/engine/upload.test.ts:43,45,48) — not 12; the test has three seam casts, not "identical four" (it never casts `editables`). The remaining ten are unrelated patterns (Record-keying styles in upload.ts:214-215, patches.ts:41, host.ts:291, signal.ts:242) and would survive the fix. One mitigation the finding omits: tsc does catch a *missing or renamed* export, since every consumer reads `generated.<name>` by name; only shape mismatches inside an existing export are invisible.
